<?php
    ...
    public function eliminar($id = NULL) {
        try {
            ...
            if (!$paciente->find_first("conditions: id=$id  and areas_id= 1"/*.Session::get('SITEID')*/)) { //si no existe
                Flash::warning("Não existe paciente com id '{$id}'");
            } else if ($paciente->delete()) {
                Flash::valid("O paciente <b>{$paciente->nombre}</b> foi excluido...!!!");
            } else {
                Flash::warning("Não é possivel excluir o paciente <b>{$paciente->nombre}</b>...!!!");
            }
