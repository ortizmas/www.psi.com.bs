...
abstract class Upload
{
    ...
    public function save($name = NULL)
    {
        ...
        if (!$name) {
            $name = ereg_replace(" ", "_",$_FILES[$this->_name]['name']);
        } else {
            $name = ereg_replace(" ", "_",$name) . $this->_getExtension();
        }
        // Guarda el archivo
    ...
    {
        if ($this->_overwrite) {
            /*$this->_name = $name . $this->_getExtension();*/
            return TRUE;
            //return FALSE;
            /*if ($this->_name === FALSE)
            {
        ...
        if (file_exists("$this->_path/$name")) {
            Flash::error('Error: ya existe este fichero. Y no se permite reescribirlo porfavor cambie de nombre.');
            return FALSE;
            //return TRUE;
