...
class AdminController extends Controller
{
    ...
    protected function _tienePermiso()
    {
        ...
        if (!$acl->check()) {
            ...
            Flash::error('Você não tem privilégios para acessar <b>' . Router::get('route') . '</b>');
            View::select(NULL);
            return FALSE;
        } else {
            $acl->resetearIntentos();
    ...
    protected function _logueoValido($user, $pass, $encriptar = TRUE)
    {
        if (MyAuth::autenticar($user, $pass, $encriptar)) {
            Flash::info('Bem-vindo ao Sistema <b>' . h(Auth::get('nombres')) . '</b>');
            return $this->_tienePermiso();
        } else {
            Input::delete();
            Flash::warning('os dados são inválidos');
            View::select(NULL, 'backend/logueo');
            return FALSE;
        }
    }
    ...
    {
        Flash::warning('Você excedeu o limite de tentativas fracassadas ao tentar acessar certas partes do sistema');
        return $this->logout();
    }
